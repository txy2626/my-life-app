...
with tab1:
    with st.form("new_log", clear_on_submit=True):
        ...
        with col_a:
            ...
            pic = st.file_uploader("配图", type=['jpg','png','jpeg'], key="log_up")
        with col_b:
...
with tab3:
    ...
    with st.expander("📝 绘制新的计划"):
        with st.form("plan_form", clear_on_submit=True):
            p_title = st.text_input("计划标题 (例如: 2026 环游世界)")
            p_details = st.text_area("详细描述/执行步骤")
        ...
        for _, row in df_plans.iterrows():
            with st.container():
                ...
                with c2:
                    ...
                    st.info(row['details'])
                    if st.button("标记已完成/删除计划", key=f"p_del_{row['id']}"):
                        with get_connection() as conn:
                            conn.execute("DELETE FROM plans WHERE id=?", (row['id'],))
                        st.rerun()
            st.divider()
    else:
        st.info("尚无计划，写下你对未来的期待吧。")
